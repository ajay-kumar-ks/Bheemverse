...
> Legend: ✅ Done | 🔄 In Progress | ⬜ Not Started | ⏭️ Skipped (deferred by project decision)
...
**Status:** ⏭️ **Skipped** — deferred; `backend/services/ai_service.py` provides rule-based stubs used by question create; full `ai-services/` package not implemented in this pass.
...
- [x] P12.1 — `frontend/src/context/AuthContext.jsx`: store `{user, token}`; expose `login()`, `logout()`, `register()`; persist token to localStorage; read on mount
- [x] P12.2 — `frontend/src/services/api.js`: axios instance with `baseURL=/api/v1`; request interceptor adds `Authorization: Bearer <token>`; response interceptor handles 401 → auto logout
- [x] P12.3 — `frontend/src/components/common/ProtectedRoute.jsx`: wraps routes requiring auth; redirects to /login if no token; `AdminRoute.jsx`: additionally checks role=admin
- [x] P12.4 — `frontend/src/pages/auth/LoginPage.jsx`: email+password form → `POST /auth/login` → store token → redirect to /dashboard; show error on 401
- [x] P12.5 — `frontend/src/pages/auth/RegisterPage.jsx`: name+email+password → `POST /auth/register` → auto-login → redirect to /dashboard
- [x] P12.6 — `frontend/src/pages/dashboard/DashboardPage.jsx`: welcome, recent questions, recent exams, stats cards (Exams Attended, Questions Created, Global Rank, Accuracy %)
- [x] P12.7 — `frontend/src/components/layout/Navbar.jsx`: logo, nav links, user avatar+dropdown (profile/logout); notification bell with unread badge (polling every 30s)

**Also delivered:** `AppLayout.jsx`, Vite `/api` proxy to backend, `AuthProvider` in `main.jsx`, protected routes for dashboard/bookmarks/notifications/profile/me/exam attend/result; `AdminRoute` for `/admin/*`.
...
- ⏭️ = Skipped by explicit project decision (tasks left unchecked)

**Session note (May 2026):** Phases P6–P11 were intentionally skipped to prioritize frontend auth (P12). Resume from P13 when continuing backend/frontend feature work.
